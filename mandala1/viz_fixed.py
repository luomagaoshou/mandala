...
def write_output(
    ...
    """
    将 dot 字符串写入文件并根据需要显示。
    
    Args:
        dot_string: 要写入的 dot 格式字符串
        output_ext: 输出文件扩展名
        output_path: 输出文件路径,如果为 None 则使用临时文件
        show_how: 显示方式,可以是 "none"、"browser"、"inline" 或 "open"
    """
    # 确保输出目录存在
    ...
    try:
        ...
        # 使用 dot 命令生成输出文件
        cmd = f"dot -T {output_ext} -o {output_path} {dot_file.name}"
        subprocess.call(cmd, shell=True)
    finally:
        # 确保文件已关闭
        if not dot_file.closed:
            dot_file.close()
        try:
            # 尝试删除临时文件
